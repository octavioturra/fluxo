    ...
    <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {Object.entries(TYPE_LABELS).map(([k, v]) => (
            <button key={k} type="button" onClick={() => setType(k as Income['type'])}
              className={`px-3 py-1.5 rounded-full text-sm font-medium ${type === k ? 'bg-[#4361EE] text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300'}`}>
              {v}
            </button>
          ))}
        </div>
      ...
      {(type === 'pj' || type === 'variavel') && (
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" checked={isEstimated} onChange={e => setIsEstimated(e.target.checked)} className="w-4 h-4 accent-[#4361EE]" />
...
export function EntradasPage() {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {/* Summary card */}
      <div className="bg-gradient-to-br from-[#22C55E] to-green-700 rounded-2xl p-4 text-white">
      ...
      {monthIncomes.length === 0 ? (
        <div className="text-center py-12 text-slate-400 text-sm">
          ...
          <button onClick={() => setShowModal(true)} className="mt-3 text-[#4361EE] font-medium">+ Adicionar entrada</button>
        </div>
      ) : (
          ...
          {monthIncomes.map(income => (
            <div key={income.id} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-4 flex items-center gap-3">
              ...
              <div className="text-right">
                <p className="font-bold text-[#4361EE]">{formatCurrency(income.amount)}</p>
              </div>
              <div className="flex gap-1 ml-1">
                <button onClick={() => setEditing(income)} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700">
                  <Pencil size={14} className="text-slate-400" />
                </button>
                <button onClick={() => { deleteIncome(income.id); toast('Entrada removida'); }} className="p-1.5 rounded-lg hover:bg-red-50">
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Nova Entrada">
        <IncomeForm month={currentMonth} year={currentYear}
          onSubmit={data => { addIncome(data); setShowModal(false); toast('Entrada adicionada'); }}
