...
export function HomePage() {
  const { user, incomes, fixedExpenses, dailyExpenses, currentMonth, currentYear, setCurrentPeriod, addDailyExpense } = useApp();
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="px-4 pb-3 grid grid-cols-3 gap-2">
        <SummaryCard label="Entradas" value={summary.totalIncome} percent={100} color="green" />
        <SummaryCard
          label="Fixos"
      ...
      <div className="px-4 pb-2">
        <button
          ...
          className="w-full flex items-center justify-between py-2 px-3 bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 text-sm font-medium text-slate-700 dark:text-slate-300"
        >
      ...
      {showCalendar && (
        <div className="px-4 pb-3">
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 overflow-hidden">
            <div className="grid grid-cols-4 text-xs font-semibold text-slate-400 uppercase px-3 py-2 border-b border-slate-100 dark:border-slate-700">
              <span>Dia</span>
              ...
              {calendar.map(day => (
                <div
                  ...
                  className={`grid grid-cols-4 text-xs px-3 py-2 border-b border-slate-50 dark:border-slate-700/50 ${
                    day.isToday ? 'bg-emerald-50 dark:bg-emerald-900/20 font-semibold' : ''
                  }`}
                >
                  <span className={`${day.isToday ? 'text-emerald-600 font-bold' : 'text-slate-500'}`}>
                    {String(day.day).padStart(2, '0')}
                    {day.isToday && <span className="ml-1 text-[10px] bg-emerald-500 text-white px-1 rounded">hoje</span>}
                  </span>
                  ...
                  <span className={`text-right font-medium ${day.balance >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
          ...
          <div className="flex gap-1">
            {(['todos', 'entrada', 'fixo', 'diario'] as FilterType[]).map(f => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`text-xs px-2 py-1 rounded-full ${filter === f ? 'bg-emerald-600 text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-500'}`}
              >
        ...
        <div className="space-y-3">
          {Object.entries(grouped).map(([date, transactions]) => (
            <div key={date}>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 overflow-hidden divide-y divide-slate-50 dark:divide-slate-700/50">
                {transactions.map(t => (
                  <div key={t.id} className="flex items-center gap-3 px-3 py-2.5">
                    ...
                    <span className={`text-sm font-semibold ${t.type === 'entrada' ? 'text-emerald-600' : 'text-slate-700 dark:text-slate-300'}`}>
...
function SummaryCard({ label, value, percent, health }: {
  label: string; value: number; percent: number; color?: string; health?: string;
}) {
  return (
